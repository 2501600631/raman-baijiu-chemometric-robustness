...
This repository contains public reproducibility and provenance materials accompanying the manuscript:

**“A robustness audit of chemometric workflows for Raman fingerprint regression of eight esters in strong-aroma Baijiu.”**

## Scope

The study evaluated a fixed Raman–chemometric workflow space for eight ester analytes in strong-aroma Baijiu. The public repository is intended to support **method transparency, provenance checking, inspection of the historical analyte-specific sample domains, and partial computational verification**.

The repository does **not** contain the underlying experimental sample matrices or the complete large derived result archives.

## Data availability

The raw experimental data are not publicly distributed because of confidentiality and data-ownership restrictions. This includes:
...
The public machine-readable manifest records the inclusion/exclusion status of the anonymized sample identifiers **S1–S80** for each analyte. It does not contain Raman intensities or GC–MS concentration values.
...
To inspect the fixed historical sample domains, open:

```text
manifests/analyte_sample_manifest.csv
```

To inspect the public MCCV screening summary, open:

```text
manifests/MCCV_screening_summary_public.csv
```

To inspect the study configuration and CRS settings, open:

```text
docs/workflow_specification.json
```

To install the captured Python package versions in a compatible Python 3.8 environment:

```bash
python -m pip install -r environment/requirements-lock.txt
```

To validate the structure and retained sample counts of the public package:
...
The public materials are instead intended to document:

1. the historical analyte-specific sample domains;
2. the MCCV screening implementation;
3. the fixed workflow design and CRS specification;
4. the captured software environment used to verify the released public scripts;
5. the integrity of the public repository snapshot.

Researchers requiring access to the underlying experimental data or large archived derived results should contact the corresponding author, subject to data-owner permission and applicable confidentiality restrictions.
...
After downloading or cloning the repository, these hashes can be used to verify that the public files have not changed relative to the recorded snapshot.
...
No license is asserted here unless a separate `LICENSE` file is added to the repository. Users should not assume rights beyond those explicitly granted by the repository owner or the associated publication.
