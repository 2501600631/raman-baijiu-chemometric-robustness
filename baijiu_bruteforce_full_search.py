...
def _script_identity() -> Dict[str, Any]:
    try:
        ...
        raise RuntimeError(
            f"无法锁定当前 Python 脚本身份，checkpoint 不允许继续：{exc}"
        ) from exc
...
def read_input_tables(spectral_file: str, phys_file: str):
    ...
    target_names = np.array([
        f"指标{i + 1}" if str(name).strip() == "" or str(name).lower() == "nan" else str(name).strip()
    ...
    if len(common) < 8:
        raise ValueError("光谱表与理化表可对齐样本少于 8 个，无法建模。")
...
def load_predeclared_exclusions(path: Optional[str]) -> pd.DataFrame:
    ...
    p = Path(path)
    if not p.exists():
        raise FileNotFoundError(f"预声明排除清单不存在：{p}")
    ...
    target_col = pick(["target", "analyte", "indicator", "指标"])
    sample_col = pick(["samplename", "sample", "sampleid", "样本", "样本编号"])
    reason_col = pick(["reason", "exclusionreason", "排除原因", "原因"])
    if target_col is None or sample_col is None:
        raise ValueError("排除清单至少需要 target 和 sampleName 两列。")
    out_df = pd.DataFrame({
...
def resolve_target_population(
    ...
    if cfg.require_reason_for_missing_reference:
        ...
        if unresolved:
            raise RuntimeError(
                f"指标 {target} 存在无参考值但无独立排除原因的样本：{unresolved}"
            )
...
class SpectralPreprocessor:
    ...
    def transform(self, X: np.ndarray):
        if not self.fitted_:
            raise RuntimeError("SpectralPreprocessor 尚未在 calibration set 上拟合。")
        out = np.asarray(X, dtype=float).copy()
...
class TargetTransformer:
    ...
    def fit(self, y_train: np.ndarray):
        ...
        if not can_apply_y_transform(y, self.method):
            raise ValueError(f"calibration y 不满足变换 {self.method} 的条件")
    ...
    def transform(self, y: np.ndarray):
        if not self.fitted_:
            raise RuntimeError("TargetTransformer 尚未在 calibration y 上拟合。")
        y = np.asarray(y, dtype=float)
        if self.method == "raw":
            return y.copy()
...
def mccv_outlier_keep(X: np.ndarray, y: np.ndarray, cfg: Config, random_state: Optional[int] = None):
    ...
    if successful != cfg.mccv_n_iter:
        raise RuntimeError(
            f"MCCV-like screening 仅完成 {successful}/{cfg.mccv_n_iter} 个成功子模型；"
            "为保持论文方法一致性，拒绝使用不完整筛选结果。"
        )

    pred_err = np.asarray(pred_err, dtype=float)
    counts = np.sum(np.isfinite(pred_err), axis=0)
    if np.any(counts == 0):
        raise RuntimeError("MCCV-like screening 中存在从未进入临时 prediction set 的样本。")
    ...
    for ratio in cfg.train_ratio_list:
        for rep in range(1, cfg.n_repeated_split + 1):
            ...
            if len(test_idx) < 5 or np.std(y[test_idx]) < 1e-12:
                raise RuntimeError(
                    f"无效 outer split: ratio={ratio}, rep={rep}, prediction set 太小或 y 无变化"
                )
            if len(train_idx) < 8:
                raise RuntimeError(
                    f"无效 outer split: ratio={ratio}, rep={rep}, calibration set 样本不足"
                )
...
class RBFLSSVMRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if best_params is None:
            raise RuntimeError("RBF-LS-SVM 没有任何完成全部 inner-CV folds 的参数组合。")
...
class ELMRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if best_params is None:
            raise RuntimeError("ELM 没有任何完成全部 inner-CV folds 的参数组合。")
...
class PCRRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if best_comp is None:
            raise RuntimeError("PCR 没有任何完成全部 inner-CV folds 的主成分数。")
...
class PLSLSSVMRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if best_params is None:
            raise RuntimeError("PLS-LSSVM 没有任何完成全部 inner-CV folds 的参数组合。")
...
def build_model(name: str, cfg: Config, random_state: int):
    ...
    if upper == "HISTGBR":
        if not HISTGBR_AVAILABLE:
            raise RuntimeError(
                "HistGradientBoostingRegressor 在当前 scikit-learn 环境中不可用。"
                "已禁止静默降级为 GradientBoostingRegressor，以避免算法名称与实际模型不一致。"
            )
    ...
    raise ValueError(f"未知或不可用算法：{name}")
...
def algorithm_list(cfg: Config):
    ...
    if missing:
        raise RuntimeError("论文要求的算法不可用：" + ", ".join(sorted(set(missing))))
    if cfg.require_exact_30_algorithms and len(algs) != 30:
        raise RuntimeError(f"论文要求恰好 30 个算法，当前实际为 {len(algs)}：{algs}")
    return algs


def validate_factorial_design(cfg: Config, algs: List[str]):
    if len(cfg.preprocess_methods) != 28:
        raise RuntimeError(f"论文要求 28 种预处理，当前为 {len(cfg.preprocess_methods)}")
    if len(cfg.target_transforms) != 4:
        raise RuntimeError(f"论文要求 4 种 y 变换，当前为 {len(cfg.target_transforms)}")
    if tuple(cfg.train_ratio_list) != (0.75, 0.80, 0.85):
        raise RuntimeError(f"论文要求训练比例 0.75/0.80/0.85，当前为 {cfg.train_ratio_list}")
    if cfg.n_repeated_split != 10:
        raise RuntimeError(f"论文要求 10 次重复划分，当前为 {cfg.n_repeated_split}")
    if len(algs) != 30:
        raise RuntimeError(f"论文要求 30 个算法，当前为 {len(algs)}")
    if str(cfg.feature_selection_mode).lower() == "fixed_count_nested" and int(cfg.fixed_feature_count) != 158:
        raise RuntimeError(f"参考结果工作簿要求固定 158 个变量，当前为 {cfg.fixed_feature_count}")
    if str(cfg.outer_split_method).lower() != "response_stratified_random":
        raise RuntimeError("正式流程必须使用 10 次响应分层随机划分。")
    ...
    if actual != expected:
        raise RuntimeError(f"factorial space 不一致：期望 {expected}，实际 {actual}")
...
def prepare_cache_for_split(X0, y0, sample_names0, wavelengths, split: SplitSpec, cfg: Config, population_meta: Dict[str, Any]):
    ...
    for y_transform in cfg.target_transforms:
        try:
            ...
            print(f"  跳过 y变换={y_transform}：{exc}")
            continue
        ...
        for method in cfg.preprocess_methods:
            try:
                ...
                if Xp_train.shape[1] < 2:
                    print(f"  跳过预处理 {method}：calibration set 有效变量少于2")
                    continue
                ...
                print(
                    f"  缓存完成：split={split.split_id:02d} | y={y_transform:8s} | "
                    f"预处理={method:20s} | 有效变量={Xp_train.shape[1]:4d} | "
                    f"固定变量={len(selected):4d} | 连续窗口={len(contiguous):4d}"
                )
            except Exception as exc:
                print(f"  预处理失败：split={split.split_id} | y={y_transform} | {method} | {exc}")
    return cache


def run_parallel_tasks(target, tasks, cfg: Config, progress_label: str = ""):
    total = len(tasks)
    print(f"开始运行指标【{target}】{progress_label}任务，总组合数：{total}")
    start = time.time()
    ...
    def _run_one(i, task):
        ...
        if cfg.show_realtime_log:
            print(
                f"[{i+1:6d}/{total:6d}] 开始：指标={target} | 算法={algorithm} | "
                f"预处理={method} | y={y_transform} | ratio={data['train_ratio']:.2f} | "
                f"rep={data['repeat_id']} | split={data['split_id']} | 特征={data['n_features']}",
                flush=True,
            )
        ...
        if cfg.show_realtime_log:
            if res is not None:
                print(
                    f"[{i+1:6d}/{total:6d}] 完成：{algorithm:18s} | {method:20s} | "
                ...
                msg = err if cfg.show_failed_reason else "失败或跳过"
    ...
    print(
        f"任务调度：普通传统模型并行 {len(classical)} | "
        f"CatBoost 串行 {len(serial_classical)} | "
        f"Torch 串行 {len(deep)}"
    )
    ...
    print(f"指标【{target}】{progress_label}完成：成功 {len(records)}/{total}，耗时 {(time.time() - start)/60:.2f} 分钟")
    return records, failures


def run_target(target, X0, y0, sample_names0, wavelengths, cfg: Config, population_meta: Optional[Dict[str, Any]] = None):
    print("=" * 80)
    print(f"正在建模指标：{target}")
    print(f"原始有效样本数：{len(y0)}")
    if len(y0) < 8 or np.std(y0) < 1e-12:
        print("跳过：样本数不足或该指标没有明显变化。")
        return [], [], None

    if cfg.outlier_filter_scope.lower() not in {"disabled", "calibration_only"}:
        raise ValueError("outlier_filter_scope 仅允许 disabled 或 calibration_only。")
    ...
    print(
        f"外部分割前固定建模总体：{len(y_model)} / {original_n}；"
        f"预声明或缺失参考值排除={population_meta['n_presearch_outliers']}"
    )
    if cfg.enable_outlier_filter and cfg.outlier_filter_scope.lower() == "calibration_only":
        print("异常筛选模式：仅 outer calibration set 内筛选；prediction set 不删除。")
    else:
        print("异常筛选模式：disabled（推荐论文主分析设置）。")

    print(
        f"算法数量：{len(algs)} | 预处理数量：{len(cfg.preprocess_methods)} | "
        f"y变换数量：{len(cfg.target_transforms)} | 训练比例数量：{len(cfg.train_ratio_list)} | "
        f"重复划分次数：{cfg.n_repeated_split}"
    )
    expected_tasks = 28 * 4 * 3 * 10 * 30
    print(f"理论组合数：{expected_tasks}")

    split_specs = build_shared_outer_splits(X_model, y_model, cfg)
    print(f"已生成并固定共享 outer splits：{len(split_specs)} 个")
    ...
    for split in split_specs:
        print("-" * 80)
        print(f"准备 split={split.split_id:02d} | ratio={split.train_ratio:.2f} | rep={split.repeat_id}")
        ...
        if cfg.require_complete_factorial and len(cache) != expected_cache:
            raise RuntimeError(
                f"split {split.split_id} 缓存路径不完整：期望 {expected_cache}，实际 {len(cache)}"
            )
        ...
        if cfg.require_complete_factorial and len(split_records) != len(tasks):
            ...
            raise RuntimeError(
                f"split {split.split_id} 未完整生成全部模型：成功 {len(split_records)}/{len(tasks)}。"
                f" 失败示例：{examples}"
            )

    records = all_records
    if cfg.require_complete_factorial and len(records) != expected_tasks:
        raise RuntimeError(f"指标 {target} 期望 {expected_tasks} 条完整记录，实际 {len(records)} 条。")
    if not records:
        print(f"指标【{target}】所有模型均失败。")
        return [], [], None
...
def export_excel(best_summary, all_records, best_by_alg_records, cfg: Config,
    ...
    if list(alg_df.columns) != METRIC_OUTPUT_COLUMNS:
        raise RuntimeError("BestByTargetAlgorithm 列结构与参考工作簿不一致。")
    if list(all_df.columns) != METRIC_OUTPUT_COLUMNS:
        raise RuntimeError("AllRunMetrics 列结构与参考工作簿不一致。")
    if list(pred_df.columns) != PREDICTION_OUTPUT_COLUMNS:
        raise RuntimeError("PredictionDetails 列结构与参考工作簿不一致。")
...
def load_checkpoint(cfg: Config, algs: Optional[List[str]] = None,
    ...
    try:
        ...
        if saved_hash != expected_hash:
            raise RuntimeError(
                "checkpoint 的科学配置与当前运行不一致，拒绝混合两套实验协议。\n"
        ...
        print("=" * 80)
        print(f"检测到兼容断点文件：{ckpt_path}")
        print(f"断点保存时间：{ckpt.get('save_time', '未知')}")
        print(f"run signature：{saved_hash}")
        print(f"已完成指标数：{len(completed_targets)}")
        if completed_targets:
            print("已完成指标：")
            for name in sorted(completed_targets):
                print(f"  - {name}")
        print("将跳过这些指标，继续训练未完成指标。")
        print("=" * 80)
        return all_records, all_best_by_alg, best_summary, completed_targets

    except Exception as exc:
        print("=" * 80)
        print(f"读取断点文件失败：{exc}")
        print("为防止混合不同实验协议，本最终版不会自动忽略不兼容 checkpoint。")
        print("请确认后删除或更名旧 checkpoint，再重新运行。")
        print("=" * 80)
        raise
...
def print_environment(cfg: Config):
    ...
    print(f"HistGBR 可用: {HISTGBR_AVAILABLE}")
    print(f"XGBoost 可用: {XGBOOST_AVAILABLE} | LightGBM 可用: {LIGHTGBM_AVAILABLE} | CatBoost 可用: {CATBOOST_AVAILABLE} | PyTorch 可用: {TORCH_AVAILABLE}")
    if TORCH_AVAILABLE:
        print(f"PyTorch CUDA 可用: {torch.cuda.is_available()}")
    print(f"暴力全组合搜索: {cfg.full_search}")
    print(f"并行 n_jobs: {cfg.n_jobs} | backend: {cfg.parallel_backend}")
    print(f"深度学习启用: {cfg.enable_deep_learning}")
    print(f"严格防泄露: {cfg.strict_leakage_guard} | shared outer splits: {cfg.shared_outer_splits}")
    print(f"外层划分: {cfg.outer_split_method} | stacking 全折内拟合: {cfg.strict_stacking_nested_preprocess}")
    print(f"变量筛选: {cfg.feature_selection_mode} | 固定变量数: {cfg.fixed_feature_count}")
    print(f"异常样本筛选范围: {cfg.outlier_filter_scope}（外部分割前不删除样本）")
    print("==============================\n")


def run_baijiu_model(spectral_file="MoNiJiu.xlsx", phys_file="HuaXueZhiBiao.xlsx", cfg: Optional[Config] = None):
    cfg = cfg or Config()
    if cfg.strict_leakage_guard and not cfg.shared_outer_splits:
        raise ValueError("strict_leakage_guard=True 时必须 shared_outer_splits=True。")
    ...
    for t, target in enumerate(target_names):
        ...
        if best_summary:
            ...
            try:
                ...
                print(f"已保存中间结果：{partial_excel}")
            finally:
                cfg.output_excel = original_excel

    if not best_summary:
        raise RuntimeError("没有任何理化指标建模成功，请检查数据格式。")
    ...
    print(f"\nExcel 结果表已生成：{cfg.output_excel}")
    print(f"断点文件：{cfg.checkpoint_file if cfg.enable_resume else '未启用'}")
    print(f"总耗时：{(time.time() - total_start) / 60:.2f} 分钟")
    return {
...
if __name__ == "__main__":
    ...
    run_baijiu_model("真酒拉曼光谱.xlsx", "8种酯类理化指标表.xlsx", config)
