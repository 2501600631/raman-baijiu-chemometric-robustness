# Raman/chemometric full-search workflow for baijiu ester prediction.
# Added comments document the scientific and computational logic only.
# No modeling logic, parameter value, data path, or output format is intentionally changed.
...
# Core numerical dependency used by preprocessing, splitting, modeling, and evaluation.
import numpy as np


# Compatibility aliases for third-party packages that may still reference deprecated NumPy names.
...
# These settings define the manuscript-aligned factorial search, leakage controls,
# feature selection, model tuning, parallel execution, checkpointing, and outputs.
@dataclass
class Config:
...
# Result container for one complete workflow evaluation.
# It stores metrics, sample membership, selected Raman variables, warnings, and timing.
@dataclass
class ModelResult:
    target: str
    model_name: str
...
# Description of one shared outer calibration/prediction split.
@dataclass
class SplitSpec:
...
# Build deterministic task-specific seeds with SHA256 so parallel task ordering cannot change randomness.
...
# SHA256 hashes lock input files, aligned matrices, script identity, and runtime context.
...
# Record software versions, platform information, parallel settings, and Torch device mode.
...
# Assemble the complete provenance record used to verify that resumed runs are scientifically identical.
...
# Serialize all scientifically relevant settings into a stable payload for run-signature hashing.
...
# --- Numerical helpers ------------------------------------------------------------------
# Metric helpers handle non-finite values defensively; complete predictions are enforced later.
...
# Synchronize NumPy and Torch random seeds for reproducible runs.
...
# --- Data loading and analyte-specific population control -------------------------------
# Spectral table: first column = Raman shift; remaining columns = sample spectra.
# Reference table: first column = analyte name; remaining columns = sample reference values.
# Samples are aligned strictly by matching sample-column names before modeling.
...
# Load independently justified exclusions; these remain distinct from model-driven outlier screening.
...
# Establish the fixed analyte-specific population before outer splitting.
# Missing references and independently predeclared exclusions are audited here.
...
# Preprocessing primitives operate on sample spectra while guarding against numerical failures.
...
# Declarative preprocessing recipes; compound methods are applied sequentially in the listed order.
_PREPROCESS_STEPS = {
    "RAW": (),
    "CENTER": ("CENTER",),
...
# Stateful calibration-fitted spectral preprocessor.
# Any learned center/scale/reference is estimated from the calibration data only and reused for prediction data,
# which is essential for preventing preprocessing leakage across the outer split.
...
# Remove non-finite or effectively constant variables using calibration-set information only.
...
# Response transformer fitted only on calibration y.
# Predictions are inverse-transformed to the original concentration scale before final metrics are computed.
...
# Optional MCCV-like calibration-only outlier screening.
# Each sample accumulates temporary holdout absolute errors; candidates exceed median + 3*MAD.
# This path is disabled in the primary release configuration.
...
# --- Nested feature selection and model-complexity control -------------------------------
# Deterministic repeated inner-CV folds are used for calibration-only tuning.
...
# Rank Raman variables by combining normalized absolute PLS coefficients (70%) and correlation scores (30%).
...
# Select a contiguous Raman interval for 1D sequence models that depend on local spectral ordering.
...
# One-standard-error rule: prefer a lower-complexity candidate if its error is within one SE of the best mean.
...
# Select variables using calibration data only; optional inner CV evaluates candidate feature counts.
...
# Nested feature-count selection beginning from raw spectra.
# Preprocessing is refitted independently in each inner fold to prevent subtle leakage.
...
# Response-stratified random outer split.
# Samples are ordered by y, partitioned into response bins, then randomly sampled within each bin.
...
# Generate the 3 calibration ratios x 10 repeats once per analyte so all workflows share identical outer splits.
...
# --- Custom regressors ------------------------------------------------------------------
# RBF LS-SVM solved directly from the least-squares support-vector linear system.
# Gamma and kernel-width parameters are chosen from complete inner-CV predictions.
...
# Extreme Learning Machine with deterministic random hidden weights and regularized output weights.
...
# Principal-component regression with inner-CV selection of PCA dimensionality.
...
# Hybrid model: supervised PLS latent variables followed by RBF LS-SVM regression.
...
# Torch architectures are defined only when PyTorch is available.
if TORCH_AVAILABLE:
...
# sklearn-compatible Torch wrapper.
# An internal validation split selects the stopping epoch; the network is then retrained on all calibration samples.
...
# Hyperparameter grids for kernel-based models; breadth is controlled by search_level.
...
# fit_best_* routines tune model complexity using calibration-only folds.
# Fold predictions are inverse-transformed and scored on the original response scale.
...
# Central estimator factory.
# Algorithms with dedicated nested-tuning functions are handled in fit_predict_base instead of here.
...
# Fit one base learner and return predictions for both calibration and prediction matrices.
...
# Refit preprocessing, variable filtering, y transformation, and feature selection independently
# inside each stacking fold before generating out-of-fold base-learner predictions.
...
# Generate complete OOF predictions for meta-learning, then refit the base learner on the full calibration set.
...
# Weight base learners by inverse squared OOF RMSE, then normalize weights to sum to one.
...
# Expanded stacking ensemble using RidgeCV as the meta-learner.
...
# Reject partial or non-finite prediction vectors rather than calculating metrics on a reduced subset.
...
# Evaluate one factorial cell: algorithm x preprocessing x response transform x outer split.
# Final Rc2/Rp2/RMSE/MAE/RPD are computed on the original response scale.
...
# Extract each algorithm's best post-selection internal-holdout result for upper-envelope reporting.
...
# Resolve optional libraries and enforce availability of all algorithms required by the declared workflow.
...
# Validate the fixed 28 x 4 x 3 x 10 x 30 factorial design = 806,400 evaluations per analyte.
...
# Prepare reusable data caches for one outer split.
# Calibration-fitted preprocessing and feature selection are computed once per preprocessing/y-transform combination
# and reused across algorithms, reducing duplicate work without exposing prediction samples to fitting.
...
# classical models run through joblib; CatBoost and Torch tasks run serially to reduce nested resource contention.
...
# Run the complete factorial experiment for one analyte across all shared outer splits.
...
# Record exact calibration/prediction sample membership for each analyte and outer split.
...
# Audit pre-search exclusions and any optional calibration-only outlier exclusions.
...
# Record selected variable indices and corresponding Raman shifts for reproducibility.
...
# Export the four-sheet compatibility workbook plus optional CSV audit artifacts.
# BestSummary/BestByTargetAlgorithm are explicitly post-selection internal-holdout summaries, not external validation.
...
# --- Checkpoint/resume ------------------------------------------------------------------
# Resume is allowed only when the scientific configuration and provenance signature exactly match.
...
# Save checkpoint state atomically through a temporary file before replacing the previous checkpoint.
...
# Print key environment and methodological settings before computation starts.
...
# Load and align data, lock provenance, resume compatible work, model each analyte, checkpoint, and export results.
...
# Standalone entry point using the manuscript-aligned release configuration.
